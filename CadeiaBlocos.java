...
public class CadeiaBlocos {
	public static void main(String[] args) throws IOException {
		...
		cadeia.add(Bloco.minerarBloco("Olá, eu sou o bloco genesis", "0"));
		cadeia.add(Bloco.minerarBloco("Olá, eu sou o segundo bloco da cadeia.", cadeia.get(cadeia.size() - 1).getHash()));
		...
		cadeia.add(Bloco.minerarBloco("Só mais dois plz!!", cadeia.get(cadeia.size() - 1).getHash()));
		cadeia.add(Bloco.minerarBloco("Pronto, agr acabou!!", cadeia.get(cadeia.size() - 1).getHash()));
		
		// Usei apenas para separar a execucao do programa.
		Scanner in = new Scanner(System.in);
		in.nextLine();
		
		// Corrompe a cadeia de blocos e depois faz alguns testes para verificar o estado da cadeia.
	...
	static boolean ValidarCadeia(ArrayList<Bloco> cadeia) {
		...
		// Verifica se a conexao entre os blocos da cadeia está valida.
		for(int i = 1; i < cadeia.size(); i++) {
			if(!cadeia.get(i).getHashAnterior().equals(cadeia.get(i - 1).getHash())) {
				System.out.println("Erro de validação, o bloco " + (i - 1) + " foi adulterado.");
				return false;
			}
		}
